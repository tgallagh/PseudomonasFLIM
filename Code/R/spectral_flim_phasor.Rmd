...
pixel_df <- read.csv(file =  '/Users/sltg/Documents/GitHub/PseudomonasFLIM/Data/Processed/df_fig2_flim.csv', header=T)
...
pixel_df$object <- ifelse( grepl(pattern='NADH', x=pixel_df$FileName, ignore.case=TRUE), "Free NADH",
                           ifelse(grepl(pattern='PVD', x=pixel_df$FileName, ignore.case=TRUE), "PVD" ,
                           ifelse( grepl(pattern = 'PYO' , x=pixel_df$FileName, ignore.case=TRUE), "PYO", NA)))
...
spectra <- ggplot() +
    geom_line(data = spectra, aes(x=Wavelength, y=Normalized, color=Sample ), size=1) +
  geom_point(data = spectra, aes(x=Wavelength, y=Normalized, fill=Sample, shape= Sample), size=2) +
  scale_fill_manual( values = color_map_1) + 
  scale_shape_manual( values = shape_maps ) +
...
flim_legend <- arrangeGrob(flim_pure, 
                           arrangeGrob( flim_pure_shape_legend, flim_pure_gradient_legend, nrow=2 ),
                           ncol=2, widths=c(4,1))
...
top_panel <- arrangeGrob(tablea, spectra, ncol=2, widths=c(1,12))
bottom_panel <- arrangeGrob(tableb, flim_legend, ncol=2, widths=c(1,12))

figure2 <- arrangeGrob(top_panel, bottom_panel, nrow=2)

ggsave(plot=figure2,filename ="/Users/sltg/Documents/GitHub/PseudomonasFLIM/Output/Paper/Figures/fig2_spectra_phasor.eps", device ='eps', height=7, width=7 )
