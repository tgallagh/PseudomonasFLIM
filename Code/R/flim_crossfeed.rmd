...
xfeed <- read.csv(file =  '/Users/sltg/Documents/GitHub/PseudomonasFLIM/Data/Processed/df_xfeed_flim.csv', header=T)
...
# get file mean 
xfeed_means <- subset( xfeed , (s1 >0) & (s1 < 0.5) & (g1 > 0) & (g1 < 1.0) )  %>%
...
all_asm <- subset(all_asm, singleton != "yes")
all_m9s <- subset(all_m9s, singleton != "yes")
all_m9s_sup <- subset(all_m9s_sup, singleton != "yes")
...
all_asm$g1_bin <- as.character(bin_data(all_asm$g1, bins = 200))
all_asm$s1_bin <- bin_data(all_asm$s1, bins = 200)
g1_bin = table(all_asm$g1_bin)
g1_bin = g1_bin[ g1_bin >100 ]
g1_bin_low <- c(row.names(g1_bin))

s1_bin = table(all_asm$s1_bin)
s1_bin = s1_bin[ s1_bin >100 ]
s1_bin_low <- c(row.names(s1_bin))
# filter out low count bins
all_asm_filter <- all_asm[ all_asm$g1_bin %in% g1_bin_low]
all_asm_filter <- all_asm_filter[ all_asm_filter$s1_bin %in% s1_bin_low]



all_m9s_sup$g1_bin <- as.character(bin_data(all_m9s_sup$g1, bins = 200))
all_m9s_sup$s1_bin <- bin_data(all_m9s_sup$s1, bins = 200)
g1_bin = table(all_m9s_sup$g1_bin)
g1_bin = g1_bin[ g1_bin >100 ]
g1_bin_low <- c(row.names(g1_bin))

s1_bin = table(all_m9s_sup$s1_bin)
s1_bin = s1_bin[ s1_bin >100 ]
s1_bin_low <- c(row.names(s1_bin))
# filter out low count bins
all_m9s_sup_filter <- all_m9s_sup[ all_m9s_sup$g1_bin %in% g1_bin_low]
all_m9s_sup_filter <- all_m9s_sup_filter[ all_m9s_sup_filter$s1_bin %in% s1_bin_low]




all_m9s$g1_bin <- as.character(bin_data(all_m9s$g1, bins = 200))
all_m9s$s1_bin <- bin_data(all_m9s$s1, bins = 200)
g1_bin = table(all_m9s$g1_bin)
g1_bin = g1_bin[ g1_bin >100 ]
g1_bin_low <- c(row.names(g1_bin))

s1_bin = table(all_m9s$s1_bin)
s1_bin = s1_bin[ s1_bin >100 ]
s1_bin_low <- c(row.names(s1_bin))
# filter out low count bins
all_m9s_filter <- all_m9s[ all_m9s$g1_bin %in% g1_bin_low]
all_m9s_filter <- all_m9s_filter[ all_m9s_filter$s1_bin %in% s1_bin_low]
...
# asm
g1_med_filter = medianFilter( all_asm_filter$g1, windowSize = 3 )
s1_med_filter = medianFilter( all_asm_filter$s1, windowSize = 3 )
g1_med_filter = medianFilter( g1_med_filter, windowSize = 3 )
s1_med_filter = medianFilter( s1_med_filter, windowSize = 3 )

all_asm_filter <- all_asm_filter[ all_asm_filter$g1 %in% g1_med_filter ]
all_asm_filter <- all_asm_filter[ all_asm_filter$s1 %in% s1_med_filter ]


# M9 succinate
g1_med_filter = medianFilter( all_m9s_filter$g1, windowSize = 3 )
s1_med_filter = medianFilter( all_m9s_filter$s1, windowSize = 3 )
g1_med_filter = medianFilter( g1_med_filter, windowSize = 3 )
s1_med_filter = medianFilter( s1_med_filter, windowSize = 3 )

all_m9s_filter <- all_m9s[ all_m9s$g1 %in% g1_med_filter ]
all_m9s_filter <- all_m9s_filter[ all_m9s_filter$s1 %in% s1_med_filter ]


# M9 succinate + sup 
g1_med_filter = medianFilter( all_m9s_sup_filter$g1, windowSize = 3 )
s1_med_filter = medianFilter( all_m9s_sup_filter$s1, windowSize = 3 )
g1_med_filter = medianFilter( g1_med_filter, windowSize = 3 )
s1_med_filter = medianFilter( s1_med_filter, windowSize = 3 )

all_m9s_sup_filter <- all_m9s_sup[ all_m9s_sup$g1 %in% g1_med_filter ]
all_m9s_sup_filter <- all_m9s_sup_filter[ all_m9s_sup_filter$s1 %in% s1_med_filter ]



all_phasor <- ggplot()+
  geom_bin2d(data = all_asm_filter, aes(x=g1, y = s1), bins=150) + 
  geom_bin2d(data = all_m9s_filter, aes(x=g1, y = s1), bins=150) + 
    geom_bin2d(data = all_m9s_sup_filter, aes(x=g1, y = s1), bins=150) +
       scale_fill_gradientn( colors = rev(yellow_blue), "Number \n of pixels")+
...
mean_phasor <- ggplot()+
    geom_point(data = xfeed_means, aes(x=mean_g1, y = mean_s1, color = media_sup)) +
    geom_point( data = ref, aes(x=mean_g1, y=mean_s1, shape=object), fill="black", size=3)+
  theme( plot.background = element_rect(fill="white", color="white"),
        panel.background = element_rect(fill="white", color="black"),
        panel.grid = element_blank(), 
...
all_g1_density <- ggplot() + geom_density(data = rbind(all_asm_filter, all_m9s_filter, all_m9s_sup_filter), aes(x = g1, fill=media_sup, color = media_sup),  size=0.5, alpha=0.3)+
  ...
  scale_fill_manual(values = media_colors ) +
   ...
   plot.title = element_text(hjust = 0.5, size=12),
        legend.title=element_blank() ,
   legend.position = 'none')+
  scale_x_continuous( breaks=pretty_breaks(n=3), limits = c(0,.75))+
  ylab("Density")


all_s1_density<- ggplot() + geom_density(data = rbind(all_asm_filter, all_m9s_filter, all_m9s_sup_filter), aes(x = s1, fill=media_sup, color = media_sup),  size=0.5, alpha=0.3)+
  scale_color_manual( values = media_colors )+
  scale_fill_manual(values = media_colors ) +
   theme( plot.background = element_rect(fill="white", color="white"),
        panel.background = element_rect(fill="white", color="black"),
...
bin_72h <- rbind(all_asm_filter, all_m9s_filter, all_m9s_sup_filter)
bin_72h <- bin_72h[ grepl(pattern = "72h", bin_72h$FileName), ]


s1_by_sample <- ggplot() + geom_density(data = bin_72h, aes(x = s1, fill=media_sup, color = FileName),alpha = 0.2)+
  scale_color_manual( values = c( rep("black", 29)) )+
  scale_fill_manual(values = media_colors ) +
   theme( plot.background = element_rect(fill="white", color="white"),
        panel.background = element_rect(fill="white", color="black"),
        panel.grid = element_blank(), 
...
ggplot() + geom_density(data = bin_72h, aes(x = g1, fill=media_sup, color = media_sup, linetype = FileName),  size=0.5, alpha=0.2)+
  scale_color_manual( values = media_colors )+
  scale_fill_manual(values = media_colors ) +
   theme( plot.background = element_rect(fill="white", color="white"),
        panel.background = element_rect(fill="white", color="black"),
...
ex_asm_mapped_filter <- subset(ex_mapped, media_sup=='ASM')
# 
# ex_asm_plot <- ggplot() + geom_bin2d(data = ex_asm_mapped, aes(x=g1,y=s1, color=media_sup), bins=200)
# 
# # extract binned data 
# ex_asm_binned <- ggplot_build( ex_asm_plot )$data[[1]]
# ex_asm_binned$media_sup <- c('ASM')
# colnames(ex_asm_binned)[c(6,7)] <- c("binned_g1", "binned_s1")
# 
# g1_med_filter = medianFilter( ex_asm_binned$binned_g1, windowSize = 3 )
# s1_med_filter = medianFilter( ex_asm_binned$binned_s1, windowSize = 3 )
# 
# 
# ex_asm_binned <- ex_asm_binned[ ex_asm_binned$binned_g1 %in% g1_med_filter, ]
# ex_asm_binned <- ex_asm_binned[ ex_asm_binned$binned_s1 %in% s1_med_filter, ]

# ex_asm_mapped_filter <- ex_asm_mapped_filter[ ex_asm_mapped_filter$s1 %in% s1_med_filter ,]

ex_asm_mapped_filter$g1_bin <- as.character(bin_data(ex_asm_mapped_filter$g1, bins = 3000))
ex_asm_mapped_filter$s1_bin <- bin_data(ex_asm_mapped_filter$s1, bins = 3000)
g1_bin = table(ex_asm_mapped_filter$g1_bin)
g1_bin = g1_bin[ g1_bin > 10 ]
g1_bin_low <- c(row.names(g1_bin))
s1_bin = table(ex_asm_mapped_filter$s1_bin)
s1_bin = s1_bin[ s1_bin >10  ]
s1_bin_low <- c(row.names(s1_bin))
# filter out low count bins
ex_asm_mapped_filter <- ex_asm_mapped_filter[ ex_asm_mapped_filter$g1_bin %in% g1_bin_low]
ex_asm_mapped_filter <- ex_asm_mapped_filter[ ex_asm_mapped_filter$s1_bin %in% s1_bin_low]


ex_mapped_filter <- rbind(ex_asm_mapped_filter, 
                          subset( ex_mapped, media_sup != 'ASM'), fill = TRUE)
...
```{r comparing mean g and s}



xfeed_means <- xfeed_means[ grepl(pattern = "72h", x = xfeed_means$FileName), ]


xfeed_means <- xfeed_means[ ( grepl(pattern = "ASM", x = xfeed_means$media_sup) | grepl(pattern = "suc", x = xfeed_means$media_sup) ), ]

xfeed_means <- xfeed_means[ !(grepl(pattern = "ASMsup", x = xfeed_means$FileName )), ]
xfeed_means <- xfeed_means[ (grepl(pattern = "Hypoxic", x = xfeed_means$FileName )), ]


xfeed<- xfeed[ grepl(pattern = "72h", x = xfeed$FileName), ]
xfeed<- xfeed[ grepl(pattern = "Hypoxic", x = xfeed$FileName), ]




  
  xfeed_phasor_mean <- ggplot() + 
  geom_path(data=subset(circle1, y>0),aes(x=x,y=y))+
  scale_color_manual( values = media_colors )+
   theme( plot.background = element_rect(fill="white", color="white"),
        panel.background = element_rect(fill="white", color="black"),
  ...
  geom_point( data = ref, aes(x=mean_g1, y=mean_s1, shape=object), fill="black", size=2)+
    geom_point(data = xfeed_means, aes(x= mean_g1, y=mean_s1, color = media_sup), size = 3 )
  
  
  mean_means <- xfeed_means %>% group_by(media_sup) %>% summarise(mean_g = mean( mean_g1), mean_s = mean(mean_s1))

  xfeed_mean_s_scatterplot <- ggplot() + 
  scale_color_manual( values = media_colors )+
   theme( plot.background = element_rect(fill="white", color="white"),
        panel.background = element_rect(fill="white", color="black"),
        panel.grid = element_blank(), 
   ...
   legend.position = 'none' ) +
    xlab(expression('G=M'%*%'cos('~phi~')'))+
  ylab(expression('S'))+
     geom_violin(data = subset(xfeed, (media_sup == "ASM" | media_sup == "M9 suc" | media_sup == "M9 suc+sup") & singleton == "no" & (s1 < 0.5) & (s1 > 0.01)) , aes(y = s1, x =media_sup))+
        geom_point(data = mean_means, aes(y = mean_s, x =media_sup), size = 6, shape = "_" )+
     scale_x_discrete(labels = function(x) str_wrap(x, width = 1))+ ylim(0,0.7)
  
  
  xfeed_mean_g_scatterplot <- ggplot() + 
  scale_color_manual( values = media_colors )+
   theme( plot.background = element_rect(fill="white", color="white"),
        panel.background = element_rect(fill="white", color="black"),
        panel.grid = element_blank(), 
   ...
   legend.position = 'none' ) +
  ylab(expression('G'))+
     geom_violin(data =  subset(xfeed, (media_sup == "ASM" | media_sup == "M9 suc" | media_sup == "M9 suc+sup") & singleton == "no" ) , aes(y =g1 , x =media_sup))+
        geom_point(data = mean_means, aes(y = mean_g, x =media_sup), size = 6, shape = "_" )+
      scale_x_discrete(labels = function(x) str_wrap(x, width = 1)) + ylim(0,1)
...
m9suc <- subset(xfeed_means, media_sup == "M9 suc")
m9sucsup <- subset(xfeed_means, media_sup == "M9 suc+sup")
asm <- subset(xfeed_means, media_sup == "ASM")


results <- t.test( m9suc$mean_g1, m9sucsup$mean_g1)
m9s_g <- data.frame( t(c( results$p.value, results$estimate, "m9suc vs sup G")))

results <- t.test( m9suc$mean_s1, m9sucsup$mean_s1)
m9s_s <- data.frame( t(c( results$p.value, results$estimate, "m9suc vs sup S")))


results <- t.test( asm$mean_g1, m9sucsup$mean_g1)
asm_g <- data.frame( t(c( results$p.value, results$estimate, "asm vs sup G")))

results <- t.test( asm$mean_s1, m9sucsup$mean_s1)
asm_s <- data.frame( t(c( results$p.value, results$estimate, "asm vs sup S")))

stat_df1 <- rbind(m9s_g, m9s_s, asm_g, asm_s)
```
...
tabled<-textGrob(c("D"), gp=gpar(fontsize=40), vjust=c(-1,0), hjust=c(.5,0))
...
panel_a <-arrangeGrob(tablea, all_phasor2, ncol=2, widths=c(1,12) )

panel_b <- arrangeGrob( tableb, arrangeGrob(xfeed_mean_g_scatterplot, xfeed_mean_s_scatterplot, widths = c(1,1)) , widths = c(1,12) )

panel_c <- arrangeGrob(tablec, ex_dc, ncol=2, widths=c(1,12))

panel_d <- arrangeGrob(tabled, 
...
ggsave(plot=panel_a,filename ="/Users/sltg/Documents/GitHub/PseudomonasFLIM/Output/Paper/Figures/fig5a_xfeed.eps", device ='eps', height=3, width=8/1.6 )

ggsave(plot=panel_b,filename ="/Users/sltg/Documents/GitHub/PseudomonasFLIM/Output/Paper/Figures/fig5b_xfeed.eps", device ='eps', height=2.5, width=8/1.6)

# ggsave(plot=panel_a_legends,filename ="/Users/sltg//Documents/GitHub/PseudomonasFLIM/Output/Paper/Figures/fig5a_xfeed_legends.eps", device ='eps', height=4, width=2 )

# ggsave(plot=all_g1_density,filename ="/Users/sltg//Documents/GitHub/PseudomonasFLIM/Output/Paper/Figures/fig5a_g1density_xfeed.eps", device ='eps', height=2/1.6, width=6/1.6 )

ggsave(plot= s1_by_sample ,filename ="/Users/sltg/Documents/GitHub/PseudomonasFLIM/Output/Paper/Figures/fig5b_s1_density_xfeed.svg", device ='svg', height=3, width=8/1.6)


ggsave(plot=panel_c,filename ="/Users/sltg//Documents/GitHub/PseudomonasFLIM/Output/Paper/Figures/fig5c_xfeed.eps", device ='eps', height=3.6/1.33, width=8/1.6 )


ggsave(plot=panel_d,filename ="/Users/sltg//Documents/GitHub/PseudomonasFLIM/Output/Paper/Figures/fig5d_xfeed.eps", device ='eps', height=4.7/1.5, width=8/1.6)


ggsave(plot=blue_orange_legend,filename ="/Users/sltg//Documents/GitHub/PseudomonasFLIM/Output/Paper/Figures/fig5c_legend.eps", device ='eps', height=1, width=4 )
